...
const HomeCareAdviceSchema = z.object({
    advice: z.string().describe("A single, safe home care suggestion."),
    youtubeLink: z.string().optional().describe("A YouTube URL for a video demonstrating the home care advice."),
});
...
const SymptomCheckerOutputSchema = z.object({
    ...
    homeCareAdvice: z.array(HomeCareAdviceSchema).describe("A list of safe, primary home care suggestions for low-severity symptoms. For each suggestion, use the 'searchYouTubeForRemedy' tool to find a relevant video. If severity is high, this should be an empty array."),
...
3.  **Suggest Home Care (for Low Severity only):** If the severity is "Low," provide a list of 2-3 safe, general home care suggestions (e.g., "Apply a warm compress," "Use artificial tears," "Gently clean the eyelid"). For each piece of advice, you MUST use the 'searchYouTubeForRemedy' tool to find a relevant video and include the link in your response.
